...
from mcp.server.minimcp.transports.starlette import (
...
STARLETTE_MODULE_NAME = "mcp.server.minimcp.transports.starlette"
...
class TestStarletteTransports:
    ...
    async def test_http_transport_success(self, mock_handler: AsyncMock, mock_request: Mock):
    ...
    async def test_http_transport_no_message_response(self, mock_request: Mock):
    ...
    async def test_http_transport_error_response(self, mock_request: Mock):
    ...
    async def test_http_transport_invalid_method(self, mock_handler: AsyncMock):
    ...
    async def test_streamable_http_transport_success(self, mock_streamable_handler: AsyncMock, mock_request: Mock):
    ...
    async def test_streamable_http_transport_with_app_transport(
        ...
        with patch(f"{STARLETTE_MODULE_NAME}.getattr", return_value=mock_transport):
            response = await streamable_http_transport(mock_streamable_handler, mock_request)

        assert isinstance(response, Response)
        mock_transport.dispatch.assert_called_once()

    async def test_streamable_http_transport_sse_response(self, mock_streamable_handler: AsyncMock, mock_request: Mock):
        ...
        with (
            patch(f"{STARLETTE_MODULE_NAME}.getattr", return_value=None),
            patch(f"{STARLETTE_MODULE_NAME}.StreamableHTTPTransport") as MockTransport,
        ):
    ...
    async def test_streamable_http_transport_no_message(self, mock_request: Mock):
    ...
    async def test_streamable_http_lifespan(self):
        ...
        with patch(f"{STARLETTE_MODULE_NAME}.StreamableHTTPTransport") as MockTransport:
            mock_transport_instance = Mock()
    ...
    async def test_http_transport_body_decoding(self, mock_handler: AsyncMock):
    ...
    async def test_streamable_http_transport_body_decoding(self, mock_streamable_handler: AsyncMock):
    ...
    async def test_streamable_http_transport_background_task_cleanup(
        ...
        with (
            patch(f"{STARLETTE_MODULE_NAME}.getattr", return_value=None),
            patch(f"{STARLETTE_MODULE_NAME}.StreamableHTTPTransport") as MockTransport,
            patch(f"{STARLETTE_MODULE_NAME}.BackgroundTask") as MockBackgroundTask,
        ):
            mock_transport_instance = Mock()
            mock_transport_instance.start = AsyncMock(return_value=mock_transport_instance)
            mock_transport_instance.aclose = AsyncMock()
    ...
    async def test_streamable_http_transport_error_handling(self, mock_request: Mock):
    ...
    def test_integration_with_starlette_app(self, mock_handler: AsyncMock):
        ...
        async def mcp_endpoint(request: Request):
            async def simple_handler(_: str) -> str:
