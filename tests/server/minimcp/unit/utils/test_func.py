...
from mcp.server.minimcp.utils.func import FuncDetails, extract_func_details, validate_func, validate_func_name

pytestmark = pytest.mark.anyio

FUNC_METADATA_PATH = "mcp.server.minimcp.utils.func.func_metadata"
VALIDATE_FUNC_PATH = "mcp.server.minimcp.utils.func.validate_func"
...
class TestExtractFuncDetails:
    ...
    def test_extract_func_details_basic_function(self):
        ...
        with patch(FUNC_METADATA_PATH) as mock_func_metadata:
            mock_meta = Mock(spec=FuncMetadata)
    ...
    async def test_extract_func_details_async_function(self):
    ...
    def test_extract_func_details_lambda_function(self):
        ...
        lambda_func: Callable[[int], int] = lambda x: x * 2  # noqa: E731
    ...
    def test_extract_func_details_calls_validate_func(self):
        ...
        with (
            patch(VALIDATE_FUNC_PATH) as mock_validate,
            patch(FUNC_METADATA_PATH) as mock_func_metadata,
        ):
            mock_meta = Mock(spec=FuncMetadata)
            mock_func_metadata.return_value = mock_meta
    ...
    def test_extract_func_details_with_invalid_function(self):
        ...
        with patch(VALIDATE_FUNC_PATH, side_effect=ValueError("Invalid function")):
...
class TestValidateFunc:
    ...
    def test_validate_func_var_positional_args_raises_error(self):
        ...
        def function_with_args(x: int, *args: Any) -> int:
            return x + sum(args)
    ...
    def test_validate_func_var_keyword_args_raises_error(self):
        ...
        def function_with_kwargs(x: int, **kwargs: Any) -> int:
            return x + len(kwargs)
    ...
    def test_validate_func_both_var_args_raises_error(self):
        ...
        def function_with_both(x: int, *args: Any, **kwargs: Any) -> int:
...
class TestIntegration:
    ...
    def test_error_propagation_from_validation(self):
        ...
        def function_with_args(*args: Any) -> int:
            return sum(args)
    ...
    def test_inspect_module_usage(self):
        ...
        with (
            patch("mcp.server.minimcp.utils.func.inspect.getdoc") as mock_getdoc,
            patch(FUNC_METADATA_PATH) as mock_func_metadata,
        ):
    ...
    def test_function_name_extraction(self):
        ...
        with patch(FUNC_METADATA_PATH) as mock_func_metadata:
            ...
            def getattr_side_effect(obj: Any, attr: str, default: None | Any = None) -> Any:
                if attr == "__name__":
                    return None
                return getattr(obj, attr, default)

            # Test with getattr returning None for __name__
            with patch("mcp.server.minimcp.utils.func.getattr", side_effect=getattr_side_effect):
